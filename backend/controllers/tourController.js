...
exports.addToWishlist = async (req, res) => {
  try {
    ...
    if (user.tourWishlist.includes(req.params.id)) {
      return res.status(400).json({
        success: false,
    ...
    user.tourWishlist.push(req.params.id);
    await user.save();

    res.status(200).json({
      success: true,
      message: 'Tour added to wishlist',
      data: user.tourWishlist
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
exports.removeFromWishlist = async (req, res) => {
  try {
    ...
    user.tourWishlist = user.tourWishlist.filter(
      tourId => tourId.toString() !== req.params.id
    );
...
exports.getWishlist = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      count: user.tourWishlist.length,
      data: user.tourWishlist
    });
  } catch (error) {
    res.status(500).json({
